...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if is_windows
    ...
    config.vm.synced_folder vconfig['aux_synced_folder'], "/var/www/vlad_aux",
      type: "smb",
      id: "vagrant-aux",
    ...
    # Setup auxiliary synced folder
    config.vm.synced_folder vconfig['aux_synced_folder'], "/var/www/vlad_aux", type: "nfs", nfs_udp: false, create: true, id: "vagrant-aux"
    ...
    # Setup auxiliary synced folder
    config.vm.synced_folder vconfig['aux_synced_folder'], "/var/www/vlad_aux", type: "rsync", create: true, id: "vagrant-aux"
  ...
  vconfig['vlad_webserver_settings'].each do |webserver_settings|
    config.hostsupdater.aliases.push('adminer.' + webserver_settings['address'])
    config.hostsupdater.aliases.push('xhprof.' + webserver_settings['address'])
    config.hostsupdater.aliases.push('logs.' + webserver_settings['address'])
    config.hostsupdater.aliases.push(webserver_settings['address'])
    webserver_settings['aliases'].each do |webserver_alias|
      config.hostsupdater.aliases.push(webserver_alias)
    end
  end
